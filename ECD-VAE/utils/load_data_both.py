...
def load_ml20m(args, **kwargs):
    ...
    def load_bg_data_popular():
        ...
        train_tensor = torch.tensor(train_data_csr, dtype=torch.float)
    
        item_popularity = torch.sum(train_tensor, dim=0)
        pop_distribution = item_popularity / torch.sum(item_popularity)
    
        top_items = int(train_tensor.shape[1] * 0.1)  
        _, top_10_percent_items = torch.topk(pop_distribution, top_items)
    
        data_bg = torch.zeros_like(train_tensor)
        sampling_count = int(train_tensor.shape[1] * 0.1)  
    
        set_seed(14)
        for i in range(train_tensor.shape[0]):
            out = torch.multinomial(pop_distribution, sampling_count, replacement=False)
            mask = torch.isin(out, top_10_percent_items)
            popular_items = out[mask]
            data_bg[i, popular_items] = 1

        rows, cols = data_bg.nonzero(as_tuple=True)
        values = data_bg[rows, cols]

        bg_train = sparse.csr_matrix((values.numpy(), (rows.numpy(), cols.numpy())), shape=train_data_csr.shape)
    ...
    def load_bg_data_unpopular():
        ...
        for i in range(train_tensor.shape[0]):
            ...
            mask = torch.isin(out, bottom_10_percent_items)
            unpopular_items = out[mask]
    ...
    bg_train_popular = load_bg_data_popular()
    bg_train_unpopular = load_bg_data_unpopular()
    ...
    y_b_popular = np.zeros((bg_train_popular.shape[0], 1))
    y_b_unpopular = np.zeros((bg_train_unpopular.shape[0], 1))

    # pytorch data loader
    train = data_utils.TensorDataset(torch.from_numpy(tg_train), torch.from_numpy(y_t_train))
    train_loader = data_utils.DataLoader(train, batch_size=args.batch_size, shuffle=True, **kwargs)

    bg_popular = data_utils.TensorDataset(torch.from_numpy(bg_train_popular.toarray()), torch.from_numpy(y_b_popular))
    popular_loader = data_utils.DataLoader(bg_popular, batch_size=args.batch_size, shuffle=True, **kwargs)

    bg_unpopular = data_utils.TensorDataset(torch.from_numpy(bg_train_unpopular.toarray()), torch.from_numpy(y_b_unpopular))
    unpopular_loader = data_utils.DataLoader(bg_unpopular, batch_size=args.batch_size, shuffle=True, **kwargs)
    ...
    return train_loader, popular_loader, unpopular_loader, val_loader, test_loader, args
...
def load_netflix(args, **kwargs):
    ...
    # train, validation and test data
    tg_train = load_train_data(os.path.join("", "Netflix", 'train.csv'))
    np.random.shuffle(tg_train)
    bg_train_popular = load_bg_data_popular()
    bg_train_unpopular = load_bg_data_unpopular()
...
def load_dataset(args, **kwargs):
    if args.dataset_name == 'ml20m':
        train_loader, popular_loader, unpopular_loader, val_loader, test_loader, args = load_ml20m(args, **kwargs)
    elif args.dataset_name == 'netflix':
        train_loader, popular_loader, unpopular_loader, val_loader, test_loader, args = load_netflix(args, **kwargs)
    else:
