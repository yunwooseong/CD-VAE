...
class VAE(Model):
    def __init__(self, args):
        ...
        self.q_z_logvar = NonLinear(self.args.hidden_size, self.args.z_size,
        ...
        self.q_s2_logvar = NonLinear(self.args.hidden_size, self.args.s1_size,
                                     activation=nn.Hardtanh(min_val=-12., max_val=4.))
        ...
        self.q_s1_logvar = NonLinear(self.args.hidden_size, self.args.s1_size,
                                     activation=nn.Hardtanh(min_val=-12., max_val=4.))
        ...
        self.p_s1_logvar = NonLinear(self.args.hidden_size, self.args.s1_size,
                                     activation=nn.Hardtanh(min_val=-12., max_val=4.))
        ...
        modules = [
            NonLinear(3 * self.args.hidden_size, self.args.hidden_size, gated=self.args.gated, activation=nn.Tanh())]
        for _ in range(0, self.args.num_layers - 2):
            modules.append(
                NonLinear(self.args.hidden_size, self.args.hidden_size, gated=self.args.gated, activation=nn.Tanh()))
    ...
    def calculate_loss(self, trainloader, bgloader, beta=1., average=False, cl_criterion=None):
        ...
        '''
        # pass through VAE
        x_mean, x_logvar, z_mean, z_logvar, tg_z, tg_z_mu, tg_z_logvar, bg_z, bg_z_mu, bg_z_logvar, tg_s1, tg_s1_mu, tg_s1_logvar, tg_s2, tg_s2_mu, tg_s2_logvar, s1_p_mean, s1_p_logvar = self.forward(
            trainloader, bgloader)  # ,tg_s

        # TG_RE
        if self.args.input_type == 'binary':
            TG_RE = log_Bernoulli(trainloader, x_mean, dim=1)
        elif self.args.input_type == 'multinomial':
            TG_RE = log_Softmax(trainloader, x_mean, dim=1)  # ! Actually not Reconstruction Error but Log-Likelihood
        ...
        if self.args.input_type == 'binary':
            ...
            BG_RE = log_Softmax(bgloader, z_mean, dim=1)  # ! Actually not Reconstruction Error but Log-Likelihood
        elif self.args.input_type == 'gray' or self.args.input_type == 'continuous':
    ...
    def forward(self, trainloader, bgloader):
        ...
        return x_mean, x_logvar, z_mean, z_logvar, tg_z, tg_z_mu, tg_z_logvar, bg_z, bg_z_mu, bg_z_logvar, tg_s1, tg_s1_mu, tg_s1_logvar, tg_s2, tg_s2_mu, tg_s2_logvar, s1_p_mean, s1_p_logvar
